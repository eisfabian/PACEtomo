...
Advanced settings:
- CTF estimation is done for every Record image and the defocus is written to the .mdoc file. CTFfind is integrated in SerialEM and is used when *doCtfFind* is set to *True*. It is fast, but not very reliable especially at higher tilt angles. IMOD's ctfplotter is more robust and now also integrated in SerialEM. You can use it by setting *doCtfPlotter* to *True*. Currently, CTF estimation is only used for *geoRefine*.
