...
### Calculate new z0
		...
		if doCtfFind:
			cfind = sem.CtfFind("A", (min(maxDefocus, trackDefocus) - 2), max(0, minDefocus + 2))
			sem.Echo("[" + str(pos + 1) + "] CtfFind: " + str(round(cfind[0], 3)))

		if doCtfPlotter:
			cplot = sem.Ctfplotter("A", (min(maxDefocus, trackDefocus) - 2), max(0, minDefocus + 2), 1, 0, pretilt)
...
### Target setup
	...
	for i, tgt in enumerate(targets):
		...
		if np.linalg.norm(np.array([tgt["SSX"], tgt["SSY"]], dtype=float)) > imageShiftLimit - alignLimit:
			sem.Echo("WARNING: Target [" + str(i).zfill(3) + "] is too close to the image shift limit. This target will we skipped.")
			skip = True
		...
		if previewAli and i != 0:								# skip for tracking target since it was already aligned after tilt to starttilt										# adds initial dose, but makes sure start tilt image is on target
			if alignToP:
				sem.V()
				sem.AlignTo("P")
				sem.GoToLowDoseArea("R")
